...
import static com.example.silkroad.domain.OrderItemAsserts.*;
...
class OrderItemResourceIT {
    ...
    public static OrderItem createUpdatedEntity(EntityManager em) {
        OrderItem updatedOrderItem = new OrderItem().quantity(UPDATED_QUANTITY).unitPrice(UPDATED_UNIT_PRICE);
        // Add required entity
        Order order;
        if (TestUtil.findAll(em, Order.class).isEmpty()) {
